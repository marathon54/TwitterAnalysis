...
tweets.hc <- twListToDF(Clinton)# convert tweets to a data frame
        for (i in c(1:2, 1500)) { #clean up wrap text feature
                cat(paste0("[", i, "] "))
                writeLines(strwrap(tweets.hc$text[i], 60))}
write.csv(tweets.hc, file = "clinton")#shortcut for saving tweets to local maching
...
#CLINTON
clinton <- read.csv("~/_data/TwitterAnalysis/clinton")#load the file saved locally
myCorpus <- Corpus(VectorSource(clinton$text))# build a corpus, and specify the source to be character vectors
